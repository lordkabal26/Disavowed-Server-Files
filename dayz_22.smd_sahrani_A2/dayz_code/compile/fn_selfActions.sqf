	...
	if (_player_lockUnlock_crtl) then {
		...
		if (s_player_copyToKey < 0) then {
			if ((_hasKey && {"ItemKeyKit" in weapons player} && {(count _temp_keys) > 1} && {!_isLocked}) || {_cursorTarget getVariable ["hotwired",false]}) then {
				s_player_copyToKey = player addAction [localize "STR_CL_VKC_CHANGE_ACTION","scripts\vkc\vehicleKeyChanger.sqf",[_cursorTarget,_characterID,if (_cursorTarget getVariable ["hotwired",false]) then {"claim"} else {"change"}],5,false,true];
			};
		};
	} else {
	...
	*/
	...
	if (Z_singleCurrency) then {
		if (_isMan && {!_isAlive} && {!(_cursorTarget isKindOf "Animal")}) then {
			if (s_player_checkWallet < 0) then {
				s_player_checkWallet = player addAction [localize "STR_CL_ZSC_CHECK_WALLET","scripts\zsc\checkWallet.sqf",_cursorTarget,0,false,true];
			};
		} else {
		...
		if ((_typeOfCursorTarget in DZE_MoneyStorageClasses || _isVehicle) && {!_isMan} && {!_isLocked} && {_isAlive} && {!(_typeOfCursorTarget in DZE_LockedStorage)}) then {
			if (s_bank_dialog < 0) then {
				s_bank_dialog = player addAction [localize "STR_CL_ZSC_ACCESS_BANK","scripts\zsc\bankDialog.sqf",_cursorTarget,1,true,true];
			};
		} else {
		...
		if (_isAlive && {_typeOfCursorTarget in AllPlayers} && {isPlayer _cursorTarget}) then {
			if (s_givemoney_dialog < 0) then {
				s_givemoney_dialog = player addAction [format [localize "STR_CL_ZSC_TRADE_COINS",CurrencyName,name _cursorTarget],"scripts\zsc\givePlayer.sqf",_cursorTarget,3,true,true];
			};
		} else {
		...
		if (Z_globalBanking) then {
			if (_isMan && {!(isPlayer _cursorTarget)} && {_typeOfCursorTarget in ZSC_bankTraders}) then {
				if (s_bank_dialog1 < 0) then {
					s_bank_dialog1 = player addAction [localize "STR_CL_ZSC_BANK_TELLER","scripts\zsc\atmDialog.sqf",_cursorTarget,3,true,true];
				};
			} else {
			...
			if (_typeOfCursorTarget in ZSC_bankObjects) then {
				if (s_bank_dialog2 < 0) then {
					s_bank_dialog2 = player addAction [localize "STR_CL_ZSC_BANK_ATM","scripts\zsc\atmDialog.sqf",_cursorTarget,3,true,true];
				};
			} else {
	...
	if (_isVehicle && {_characterID == "0"} && {"ItemKeyKit" in weapons player} && {!_isMan} && {_isAlive}) then {
		if (s_player_claimVehicle < 0) then {
			...
			if (count (_totalKeys select 0) > 0) then {
				s_player_claimVehicle = player addAction [format [localize "STR_CL_VKC_CLAIM_ACTION",_text],"scripts\vkc\vehicleKeyChanger.sqf",[_cursorTarget,_characterID,"claim"],5,false,true];
			};
		};
	} else {
	...
	if (_typeOfCursorTarget in vg_List) then {
		...
		if (s_garage_dialog < 0 && ((_hasAccess select 0) || (_hasAccess select 2) || (_hasAccess select 3) || (_hasAccess select 4) || (!isPlayer _cursorTarget && {_typeOfCursorTarget in vg_list}))) then {
			s_garage_dialog = player addAction [localize "STR_CL_VG_VIRTUAL_GARAGE","scripts\virtualGarage\virtualGarage.sqf",_cursorTarget,3,false,true];
		};
	} else {
