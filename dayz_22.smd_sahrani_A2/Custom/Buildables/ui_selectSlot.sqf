...
if (_button == 1) then {
	...
	if (_item == (primaryWeapon player) && !((primaryWeapon player) in MeleeWeapons) && _dayz_onBack == "EMPTY" && (primaryWeapon player) != "") then {
		_menu = _parent displayCtrl (1600);
		_menu ctrlShow true;
		_type = "Add to back";
		_height = _height + (0.025 * safezoneH);
		_compile = format["_id = '%1' spawn player_addWeaponToBack;",_item];
		uiNamespace setVariable ['uiControl', _control];
		_menu ctrlSetText format[_type,_name];
		_menu ctrlSetEventHandler ["ButtonClick",_compile];
	};
